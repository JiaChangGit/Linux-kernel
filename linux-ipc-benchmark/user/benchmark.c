...
static void *syscall_worker(void *arg)
{
    ...
    for (i = 0; i < a->count; i++) {
        if (a->is_producer) {
            ...
            } while (r < 0 && (errno == EAGAIN || errno == ENOSPC
                                || errno == EINTR));
        } else {
            do {
...
int main(int argc, char *argv[])
{
    ...
    if (fd_mq < 0) { perror("open " MQ_DEVICE); return 1; }
    fd_shm = open(SHM_DEVICE, O_RDWR);
    if (fd_shm < 0) { perror("open " SHM_DEVICE); close(fd_mq); return 1; }

    shm = mmap(NULL, SHM_MAP_SIZE, PROT_READ|PROT_WRITE, MAP_SHARED, fd_shm, 0);
    if (shm == MAP_FAILED) { perror("mmap"); goto done; }
    ...
    printf("    producer : %8.1f ms  →  %12.0f msg/s\n",
           prod[0]/1e3, (double)count/(prod[0]/1e6));
    printf("    consumer : %8.1f ms  →  %12.0f msg/s\n",
           cons[0]/1e3, (double)count/(cons[0]/1e6));
    ...
    printf("    producer : %8.1f ms  →  %12.0f msg/s\n",
           prod[1]/1e3, (double)count/(prod[1]/1e6));
    printf("    consumer : %8.1f ms  →  %12.0f msg/s\n",
           cons[1]/1e3, (double)count/(cons[1]/1e6));
    printf("    wall     : %8.1f ms  →  %12.0f msg/s  (combined)\n\n",
           wall[1]/1e3, tp[1]);

    /* ━━━━ Test 3 : SHM mmap zero-copy ━━━━ */
    shm->head = shm->tail = 0;          /* reset ring before this test     */
    sep();
    printf("[3] Shared Memory  (ring-buf + mmap,  ZERO-COPY)\n");
    printf("    copy path: producer memcpy→page  consumer memcpy←page\n");
    printf("                *** no copy_from/to_user, no per-msg syscall ***\n\n");
    wall[2] = run_test(fd_shm, count, 1, shm, &prod[2], &cons[2]);
    tp[2]   = (double)count / (wall[2] / 1e6);
    printf("    producer : %8.1f ms  →  %12.0f msg/s\n",
           prod[2]/1e3, (double)count/(prod[2]/1e6));
    printf("    consumer : %8.1f ms  →  %12.0f msg/s\n",
           cons[2]/1e3, (double)count/(cons[2]/1e6));
    printf("    wall     : %8.1f ms  →  %12.0f msg/s  (combined)\n\n",
           wall[2]/1e3, tp[2]);
    ...
    system("cat /proc/mq_stats  | grep -E 'avg_latency|enqueue|dequeue' "
           "| sed 's/^/    /'");
    printf("  /proc/shm_stats:\n");
    system("cat /proc/shm_stats | grep -E 'avg_latency|write|read_count'  "
           "| sed 's/^/    /'");
    printf("\n");
